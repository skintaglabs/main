    ...
    <div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
        {warningSign.map(({ letter, word, hint }) => (
          <div
            key={letter}
            className="bg-[var(--color-surface)] border rounded-[var(--radius)] p-4 transition-all hover:shadow-[var(--shadow)] hover:-translate-y-0.5 cursor-default"
          >
            <div
              className="text-[32px] leading-none font-semibold mb-2"
              style={{ fontFamily: "'Instrument Serif', serif" }}
            >
              {letter}
            </div>
            <div className="text-[15px] font-medium mb-1">{word}</div>
